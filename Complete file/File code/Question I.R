...
new_df$sqft_living<-log(new_df$sqft_livi?g)
...
## p-value ?f all independent variable is 2e-16 < 0,05 = alpha, so the all independent variable are meaningful.
...
SSR <- ((linearModule$fitted.value? - mean(new_df$price)) ^ 2) %>% sum()
